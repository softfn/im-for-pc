...
public class WindowAutoHide implements MouseMotionListener, ActionListener, ComponentListener, WindowStateListener {
	...
	public WindowAutoHide(final Window window) {
		this.window = window;
		this.step = 10;
		this.rim = 2;
		this.hideDelay = 230;
		...
		if (window.isVisible()) {
			...
			window.addComponentListener(new ComponentAdapter() {
				...
				public void componentShown(ComponentEvent e) {
					...
					WindowAutoHide.this.componentShown(null);
				}
			});
		}
	}
	...
	private void addOrRemoveListener(boolean add) {
		if (!add && listenerAdded) {
			window.removeMouseMotionListener(this);
			window.removeComponentListener(this);
			listenerAdded = false;
		} else if (add && !listenerAdded) {
	...
	private void stopHideTimer() {
		...
		if (force) {
			...
			this.mouseIn = window.getBounds().contains(mouseLocation);
			int winState = Frame.NORMAL;

			if (window instanceof Frame) {
				winState = ((Frame) window).getExtendedState();
			}

			if (window.isVisible() && winState == Frame.NORMAL) {
				mouseTimer.start();
			}
		}
	}
