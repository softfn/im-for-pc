 ...
 * @description:
 * @author XiaHui
 * @date 2014年6月26日 上午10:11:52
 * @version 1.0.0
 */
public class OnlyImageUtil {
	/**
	 * 根据路径获取ImageIcon
	 * 
	 * @param imagePath图片路径
	...
	public static BufferedImage getRoundedCornerBufferedImage(String imagePath, int width, int height, int cornersWidth, int cornerHeight) {
		try {
			...
			Logger.getLogger(OnlyImageUtil.class.getName()).log(Level.SEVERE, null, ex);
		}
		return null;
	}
	...
	public static BufferedImage getBufferedImage(String image, int alpha) {
		if (null != image && !"".equals(image)) {
			...
			try {
				// 读取图片
				FileInputStream stream = new FileInputStream(new File(image));// 指定要读取的图片
	 ...
	 *            0.0为完全透明，1.0为完全不透明)
	 */
	public static void pressText(String imagePath, String pressText, String fontName, int fontStyle, int fontSize, Color color, int x, int y, float alpha) {
		try {
