...
import com.over.uitl.GraphicsUtils;
import com.over.uitl.ImageFilterUtils;
import com.over.uitl.ImageUtils;

/**
 * User: mgarin Date: 05.06.12 Time: 16:23
 */

public class OverDecoratedImage extends JComponent implements SwingConstants, ShapeProvider {
	...
	// Preview alignment
	private int horizontalAlignment = OverDecoratedImageStyle.horizontalAlignment;
	private int verticalAlignment = OverDecoratedImageStyle.verticalAlignment;

	// Basic effects
	private boolean drawBorder = OverDecoratedImageStyle.drawBorder;
	private Color borderColor = OverDecoratedImageStyle.borderColor;
	private boolean drawGlassLayer = OverDecoratedImageStyle.drawGlassLayer;
	private int shadeWidth = OverDecoratedImageStyle.shadeWidth;
	private int round = OverDecoratedImageStyle.round;

	// Filters
	private float blurAlignX = OverDecoratedImageStyle.blurAlignX;
	private float blurAlignY = OverDecoratedImageStyle.blurAlignY;
	private boolean grayscale = OverDecoratedImageStyle.grayscale;
	private boolean blur = OverDecoratedImageStyle.blur;
	private float blurFactor = OverDecoratedImageStyle.blurFactor;
	private boolean zoomBlur = OverDecoratedImageStyle.zoomBlur;
	private float zoomBlurFactor = OverDecoratedImageStyle.zoomBlurFactor;
	private boolean rotationBlur = OverDecoratedImageStyle.rotationBlur;
	private float rotationBlurFactor = OverDecoratedImageStyle.rotationBlurFactor;

	public OverDecoratedImage() {
		this((ImageIcon) null);
	}

	public OverDecoratedImage(String src) {
		this(ImageUtils.loadImage(src));
	}

	public OverDecoratedImage(Class<?> nearClass, String src) {
		this(ImageUtils.loadImage(nearClass, src));
	}

	public OverDecoratedImage(Image image) {
		super();
		setImage(image);
	}

	public OverDecoratedImage(ImageIcon icon) {
		super();
		setIcon(icon);
	}
	...
	public void updatePreview() {
		...
		if (round > 0) {
			image = ImageUtils.cutImage(new RoundRectangle2D.Double(0, 0, icon.getIconWidth(), icon.getIconHeight(), round * 2, round * 2), image);
		}
		...
		if (shadeWidth > 0 || drawGlassLayer || drawBorder) {
			...
			// Shade
			GraphicsUtils.drawShade(g2d, bs, OverDecoratedImageStyle.shadeType, new Color(90, 90, 90), shadeWidth);
			...
			// Glass-styled shade
			if (drawGlassLayer) {
				g2d.setPaint(new GradientPaint(0, shadeWidth, new Color(255, 255, 255, 160), 0, shadeWidth + (ps.height - shadeWidth * 2) / 2, new Color(255, 255, 255, 32)));
	...
	private Shape getGlanceShape() {
		Dimension ps = getPreferredSize();
		if (round > 0) {
			return new RoundRectangle2D.Double(shadeWidth, shadeWidth, ps.width - shadeWidth * 2, (ps.height - shadeWidth * 2) / 2, round * 2, round * 2);
		} else {
